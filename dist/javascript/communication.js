class Communication{
    ...
    attempt_connection(what_to = '', address){
      return new Promise((resolve,reject) => {
        ...
        // Log
        this.connection_log.add(`Attempting connection to ${what_to} <kbd>${address}</kbd>`, {icon: 'stethoscope', color: 'default'});
        ...
        this.command(['version', 'M115'], false, false, address).then((result) => {
          ...
          this.connection_log.add(`Succesful connection to <kbd>${address}</kbd>`, {icon: 'thumbs-up', color: 'success'});
          resolve(result);
        }).catch((data) => {
          // We got no answer
          this.connection_log.add(`No answer from <kbd>${address}</kbd>`, {icon: 'phone-slash', color: 'warning'});
          reject();
        });
      ...
      return new Promise((resolve,reject) => {
        ...
        if( address_list.length === 0 ){
          ...
          if( !window.location.pathname.match(/^\/connect\.html/) ){
            // Log
            this.connection_log.add(`Could not connect, going to the connection page to allow user to fix the situation`, {icon: 'fighter-jet', color: 'default'});
          ...
          this.attempt_connection(target.to_what, target.address).catch(() => {
            ...
            this.try_address_list( address_list ).catch((reason) => {reject(reason)});
          }).then((result) => {
    ...
    found_board(address, data){
      ...
      setTimeout(() => {
        this.connected = true;
        this.on_connected.forEach((callback) => {callback.call();});
      }, 1000);
    ...
    upload( file, vue ){
      ...
      reader.onloadend  = (evt) => {
          ...
          // for formatting
          var formatBytes = function(a,b){if(0==a)return"0 Bytes";var c=1024,d=b||2,e=["B","kB","MB","GB","TB","PB","EB","ZB","YB"],f=Math.floor(Math.log(a)/Math.log(c));return parseFloat((a/Math.pow(c,f)).toFixed(d))+" "+e[f]}
          ...
          eventSource.addEventListener("progress", function(e) {
              ...
              var average = speeds.reduce((total, num) => {return total + num; }) / speeds.length;

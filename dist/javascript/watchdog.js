...
class WatchDog{
  ...
  constructor(parent){
    this.machine = parent;
    this.temperatures = {};
    this.work_position = {x: 'error', y: 'error', z: 'error'};
    this.machine_position = {x: 'error', y: 'error', z: 'error'};
    this.working = false;
  }
  ...
  get_board_status(){
    ...
    this.machine.communication.get_status().then((data) => {
      ...
      var parse_position = (position) => { var pos = position.replace(/^[MW]Pos:/g,'').split(','); return {x: pos[0], y: pos[1], z: pos[2]}; };
      ...
      while(status.length){
        ...
        this.temperatures[p[1]] = {temperature:p[2], target:p[3]};
